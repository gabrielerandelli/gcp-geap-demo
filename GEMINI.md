...
- **Google ADK skills** (already available in this environment). Use them instead of guessing at ADK APIs:
...
- **Agent Identity has TWO distinct surfaces** — don't confuse them:
  - **Auth manager** (`agentidentity.googleapis.com` + `agentidentitycredentials.googleapis.com`) — brokers credentials for external APIs. Three types: `apiKey`, `twoLeggedOauth`, `threeLeggedOauth`. Provisioned via `gcloud alpha agent-identity auth-providers create` → returns a `projects/{p}/locations/{l}/authProviders/{id}` resource. Wired in ADK via `AuthenticatedFunctionTool(func=…, auth_config=AuthConfig(auth_scheme=GcpAuthProviderScheme(name=<resource>)))` — the runtime SA needs `roles/agentidentity.user` (or `roles/iamconnectors.user`; both exist due to renames). At tool-invoke time the ADK's `_AgentIdentityCredentialsProvider` calls `AuthProviderCredentialsService.retrieve_credentials`. **AuthConfig import path is `google.adk.auth.auth_tool`, not `google.adk.auth.auth_config`** (the ADK integration page's example is stale). **For apiKey providers, the credential lands in `credential.http.additional_headers`** (both under a provider-set custom header name and under `X-GOOG-API-KEY`); only `Bearer` tokens use `credentials.token`.
  - **Workload identity** (GA — SPIFFE `principal://agents.global.org-<ORG>.system.id.goog/resources/…`) — enabled per-deploy via `identity_type=AGENT_IDENTITY`. Grantable directly on target GCP resources (e.g. `run.invoker` on a Cloud Run service). This project's math-agent-to-MCP still uses the SA + ID-token path; workload identity is a candidate future upgrade.
  - Choose (a) when the downstream is a third-party or Google API needing API-key/OAuth. Choose (b) when the downstream is a sibling GCP resource that accepts IAM bindings on a `principal://…` identifier. Same-project Cloud Run is the (b) case — don't try to shoehorn it through the auth manager.
